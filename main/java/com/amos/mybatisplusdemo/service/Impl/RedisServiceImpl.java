package com.amos.mybatisplusdemo.service.Impl;
...
public class RedisServiceImpl implements RedisService {
    ...
    private static final int seconds = 3600 * 24;
        ...
        return redisTemplate.execute(new RedisCallback<List<T>>() {
            ...
            public List<T> doInRedis(RedisConnection connection) throws DataAccessException {
                ...
                for (String newKey : keysSet) {
                    data = connection.hGetAll(Objects.requireNonNull(serializer.serialize(newKey)));
                    result = new HashMap<>();
                    assert data != null;
                    for (Map.Entry<byte[], byte[]> entry : data.entrySet()) {
                        result.put(serializer.deserialize(entry.getKey()), serializer.deserialize(entry.getValue()));
    ...
    public String lpop(final String key) throws Exception {
        ...
        return redisTemplate.execute(new RedisCallback<String>() {
            ...
            public String doInRedis(RedisConnection connection) throws DataAccessException {
                RedisSerializer<String> serializer = redisTemplate.getStringSerializer();
                byte[] res = connection.lPop(Objects.requireNonNull(serializer.serialize(key)));
